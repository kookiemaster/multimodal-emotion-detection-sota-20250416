"""
Main script for training and evaluating the MemoCMT model on ESD dataset.

This script implements the complete pipeline for replicating the MemoCMT model
results on the ESD dataset as described in the paper.
"""
...
from src.memocmt.data.data_pipeline import prepare_esd_data
from src.memocmt.models.training import train_memocmt_esd
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("memocmt_esd.log"),
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Train and evaluate MemoCMT model on ESD dataset")
    
    parser.add_argument("--data_dir", type=str, default="data/processed/esd",
                        help="Directory containing processed ESD data")
    parser.add_argument("--output_dir", type=str, default="results/memocmt_esd",
...
def main():
    """Main function for training and evaluating MemoCMT on ESD."""
    # Parse arguments
    args = parse_args()
    ...
    # Define emotion mapping for ESD
    emotion_map = {
        'neutral': 0,
        'happy': 1,
        'angry': 2,
    ...
    # Prepare data
    logger.info(f"Preparing ESD data from {args.data_dir}...")
    try:
        train_loader, val_loader, test_loader = prepare_esd_data(
        ...
        logger.info(f"Data preparation complete. Train: {len(train_loader.dataset)} samples, Val: {len(val_loader.dataset)} samples, Test: {len(test_loader.dataset)} samples")
    ...
    try:
        history, test_acc = train_memocmt_esd(
    ...
    try:
        ...
        if os.path.exists(predictions_path):
            ...
            published_results = get_published_results("memocmt", "esd")
